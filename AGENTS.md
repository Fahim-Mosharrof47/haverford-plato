...
| `CompanionScreenCaptureUtility.swift` | ~260 | Multi-monitor screenshot via ScreenCaptureKit (capped at 1600px, was 1280px). Filters out own app's windows, sorts displays by cursor position, returns AppKit coordinates. `captureNativeResolutionCrop` cuts a native-res close-up for crop re-localization (`point_in_crop`). |
| `OpenAIRealtimeClient.swift` | ~1220 | OpenAI Realtime WebSocket client. Connects to `wss://api.openai.com/v1/realtime`. Handles audio in/out, screenshots, session pre-warm, usage reporting via `response.done`. Events published via `PassthroughSubject`. |
...
| `RealtimeTelemetry.swift` | ~480 | JSONL telemetry logger for Realtime sessions. Per-turn rows (token counts, timing, speech durations, vision usage, pointing rollup) and session summary written to `~/Library/Application Support/skilly-telemetry.jsonl`. Also forwards aggregate metrics to PostHog. |
| `PointingInstrumentation.swift` | ~220 | Pure pointing-measurement primitives (root-cause report `docs/reviews/2026-07-07-pointing-root-cause-report.md`): `PointAnchorPath`/`PointOutcome` (which source anchored each visual + offset from the model's guess), `PointDeclineGate`/`PointDecline` (which gate silently dropped a visual), `WhereIsIntentClassifier` (did the user ask to locate something), `PointingTurnMetrics` per-turn accumulator. Rollups land in the telemetry turn row and PostHog `point_*` events. |
